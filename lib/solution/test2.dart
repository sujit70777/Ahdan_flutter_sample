...
PrayTimes.dart: Prayer Times Calculator (ver 2.3) - Dart Port
Original Copyright (C) 2007-2011 PrayTimes.org
Dart Port: Converted from JavaScript
...
class PrayTimes {
  ...
  double sunAngleTime(double angle, double time, [String? direction]) {
    // print(
    //   'Calculating sun angle time for angle: $angle, time: $time, direction: $direction',
    // );
    Map<String, double> sunPos = sunPosition(_jDate + time);
  ...
  double asrTime(double factor, double time) {
    ...
    return sunAngleTime(
      angle,
      time,
    ); // No direction parameter - defaults to clockwise
  }
  ...
  double adjustHLTime(
    ...
    double timeDiffValue =
        (direction == 'ccw')
            ? this.timeDiff(time, base)
            : this.timeDiff(base, time);
    if (time.isNaN || timeDiffValue > portion) {
...
class DMath {
  ...
  static double fix(double a, double b) {
    if (a.isNaN) return a; // Handle NaN
    a = a - b * (a / b).floor();
    return (a < 0) ? a + b : a;
  }
}
...
//   Map<String, String> times = pt.getTimes(DateTime.now(), [43.0, -80.0], -5);
